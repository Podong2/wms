...
//            .orElseGet(() -> new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR));
    ...
    @RequestMapping(value = "/users/findByNameAndEntityId",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<UserDTO>> findByNameAndExcludeIds(@RequestParam("name") String name
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the "login" user, or with status 404 (Not Found)
     */
    @RequestMapping(value = "/users/findProjectMember",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<UserDTO>> findProjectMember(@RequestParam(value = "name", required = false) String name, @RequestParam("projectId") Long projectId
        ...
        List<UserDTO> users = userService.findProjectMember(name, projectId, excludeIds);
